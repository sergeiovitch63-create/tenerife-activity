/**
 * ContextualBanner — server-rendered home advisory.
 *
...
export function ContextualBanner({ title, body }: Props) {
  ...
  return (
    <div className="container-x -mt-6 relative z-10">
      <div className="rounded-2xl border border-ember-200 bg-ember-50 px-4 py-3 md:px-5 md:py-4 flex items-start gap-3">
        <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ember-500/15 text-ember-700">
          <AlertTriangle className="h-4 w-4" />
        </span>
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold text-ember-900">{title}</div>
          <p className="mt-0.5 text-sm text-ember-800/90">{body}</p>
        </div>
      </div>
    </div>
  )
