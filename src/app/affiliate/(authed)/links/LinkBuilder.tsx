...
export default function LinkBuilder({ code, siteUrl }: { code: string; siteUrl: string }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Langue du visiteur</label>
        <select
          value={locale}
          onChange={(e) => setLocale(e.target.value)}
          className="block w-full sm:w-48 rounded-md border border-gray-300 px-3 py-2 text-sm"
        >
      ...
      <div className="border-t border-gray-100 pt-5 space-y-3">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Ton lien</label>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={copy}
              className="rounded-md bg-blue-600 text-white px-4 py-2 text-sm hover:bg-blue-700 whitespace-nowrap"
            >
              {copied ? '✓ Copié' : 'Copier'}
            </button>
          </div>
