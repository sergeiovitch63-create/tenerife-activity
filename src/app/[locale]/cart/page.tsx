 ...
 * Real names, dates, participants, images (same as activity cards).
 */
    ...
    const run = async () => {
      try {
        const res = await fetch(
          `/api/atlantico/group-details/${encodeURIComponent(tGroup)}/${encodeURIComponent(lang)}`
        )
...
function CartItemCard({ item, currency, onRemove, onUpdate, t, lang }: CartItemCardProps) {
  ...
  const displayName = useGroupName(item.t_group, item.language, item.tourName || '')
